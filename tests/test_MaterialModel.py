    ...
    def compute_stress(self, de):
        ...
        for i in range(self.n - 1):
            ...
            if phi_trial <= 0:  # Elastic step
                ...
                sig[i + 1] = sig_trial - np.sign(sig_trial) * self.E * dep
                ep[i + 1] = ep[i] + dep
    ...
    def compute_stress(self, de):
        ...
        for i in range(self.n - 1):
            ...
            # Plastic step
                ...
                sig[i + 1] = sig_trial - np.sign(etha_trial) * self.E * dep  # Update stress
                alpha[i + 1] = alpha[i] + np.sign(etha_trial) * self.H * dep  # Update backstress
                ep[i + 1] = ep[i] + dep  # Increment plastic strain
